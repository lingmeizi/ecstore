...
class b2c_finder_members{
    ...
    var $column_member_name = '上级用户名';
    public function column_member_name($row){
        $userObject = kernel::single('b2c_user_object');
        $member_model = $this->app->model('members');
        $members = $member_model->getRow("up_member_id",array("member_id"=>$row['member_id']));
        if($members['up_member_id']>0)
        {
        $member_name = $userObject->get_member_name(null,$members['up_member_id']);
        }
        return $member_name;
    }
