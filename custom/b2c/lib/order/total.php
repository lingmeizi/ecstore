...
class b2c_order_total
{
    ...
    public function order_total_method(&$controller,$cart,$sdf_order,$need_pmt_info='false',$view='site/cart/checkout/checkout_total.html')
    {
        ...
        //添加预售信息2行方便前台显示
        $cart['prepare']['nowtime']=time();
        $payment_detail['prepare']=$cart['prepare'];
        $payment_detail['promotion_type']=$cart['promotion_type'];
        $payment_detail['shipping_id']=$sdf_order['shipping_id'];
    ...
    public function payment_detail(&$controller,$cart,$sdf_order)
    {
        ...
        $member_up = app::get("b2c")->model('members')->getRow('up_member_id,isjxs',array('member_id'=>$sdf_order['member_id']));
        if(!empty($member_up["up_member_id"])&&$member_up["up_member_id"]>0&&$member_up["isjxs"]==0&&$total_amount_odr>0&&$total_amount_odr>$cart_info['discount_useramount'])//会员购买按折扣计算
        {
         $total_amount_odr=$total_amount_odr-$cart_info['discount_useramount'];
        }
		...
		$cost_item = $objMath->number_minus(array($cost_item, $cart_info['discount_amount_prefilter']));
                $payment_detail = array('cost_item'=>$objCurrency->amount_nocur($cost_item, $sdf_order['cur'], false, false),
                                ...
                                'pmt_discount' =>$cart_info['discount_useramount'],
