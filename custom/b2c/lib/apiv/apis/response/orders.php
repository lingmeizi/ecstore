...
class b2c_apiv_apis_response_orders
{
     ...
     * @return boolean true or false
     */
    public function SOOutputCustoms(&$sdf)
    {
       // MerchantOrderID--Status==ShipTypeID(物流公司编号)==TrackingNumber==CommitTime==Message
        if(!$sdf)
        {
            return "FAILURE";
        }
        ...
        $arr_data['ship_status'] = 1;//发货状态自己改成已发货
        $arr_data['order_id'] = $sdf['MerchantOrderID'];

        //$arr_order = $order->dump($sdf['MerchantOrderID']);
        $arr_order=$order->getRow("ship_status,payment,shipping_id,cost_freight,is_protect,cost_protect,ship_name,ship_mobile,ship_tel,ship_addr,ship_zip,ship_area",array("order_id"=>$sdf['MerchantOrderID']));
        if ($arr_order)
        {
            //生成发货单
            $freez_time=app::get("b2c")->getConf('system.goods.freez.time');
            $db = kernel::database();
            $sdf_data=array(
                'order_id' =>$sdf['MerchantOrderID'],
                'pay_app_id' => $arr_order["payment"],//支付方式
                'freez_time' => $freez_time,
                'delivery' =>$arr_order["shipping_id"],//配送方式
                'logi_id' => $this->getWl($sdf['ShipTypeID']),//物流公司
                'logi_no' => $sdf['TrackingNumber'],//物流编号
                'money' => $arr_order["cost_freight"],//配送费用
                'is_protect' => $arr_order["is_protect"],//是否还有保价费
                'cost_freight' => $arr_order["cost_protect"],//保价费
                'ship_name' => $arr_order["ship_name"],
                'ship_tel' => $arr_order["ship_tel"],
                'ship_mobile' => $arr_order["ship_mobile"],
                'ship_zip' => $arr_order["ship_zip"],
                'ship_area' => $arr_order["ship_area"],
                'ship_addr' => $arr_order["ship_addr"],  
                'opid' =>'1',
                'opname' =>'admin',
            );
            //如果发货状态为已发货，自己返回失败
            ...
            $transaction_status = $db->beginTransaction();
            $obj_checkorder = kernel::service('b2c_order_apps', array('content_path'=>'b2c_order_checkorder'));
            if (!$obj_checkorder->check_order_delivery($sdf_data['order_id'],$sdf_data,$message))
            {
                $db->rollback();
                 return "FAILURE";
            }
            // 处理支付单据.
            $items=$orderItemModel->getList("product_id,nums,goods_id,item_id",array("order_id"=>$sdf_data["order_id"]));
            foreach ($items as $k=> $v) {
                $itemarry[$v["item_id"]]=$v["nums"];
            }
            //查询订单明细
            $sdf_data["send"]=$itemarry;
            $objB2c_delivery = b2c_order_delivery::getInstance(app::get("b2c"), app::get("b2c")->model('delivery'));
            //$controller= kernel::single('b2c_ctl_admin_order');
            if ($objB2c_delivery->generate($sdf_data, $this, $message))
            {
                 $db->commit($transaction_status); 
                 //更新会员信息推送到crm
                if($order_object = kernel::service('b2c_order_rpc_async')){
                    $order_object->modifyActive($sdf_data['order_id']);
                }
                //自动确定收货操作
                $obj_delivery_time = app::get('b2c')->model('order_delivery_time');
                $arr_delivery_time = array('order_id'=>$sdf_data['order_id'],'delivery_time'=>time()+10*24*3600);
...
//            {
//                return "FAILURE";
//                //$thisObj->send_user_error(app::get('b2c')->_('订单发货状态修改失败！'), array('tid'=>$sdf['MerchantOrderID']));
//            }
        }
        else
        {
            return "FAILURE";
            //$thisObj->send_user_error(app::get('b2c')->_('订单不存在！'), array('tid'=>$sdf['MerchantOrderID']));
        }
      }
//      else{
//          return "FAILURE";
//          //$thisObj->send_user_error(app::get('b2c')->_('订单出关失败！'), array('tid'=>$sdf['MerchantOrderID']));
//      }
    }
    
    //物流公司判断
    private function getWl($code)
    {
      switch ($code)
        {
            case 1:
