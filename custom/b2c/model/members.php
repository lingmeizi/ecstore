...
class b2c_mdl_members extends dbeav_model{
    ...
    //添加验证
     function validate(&$data,&$msg){
       $fag = 1;
       if($data['is_salesman']){
           $type_id=$data['member_type']['member_type_id'];
           $profit_id=$data['member_profit']['member_profit_id'];
           if(($type_id=='0'||$type_id=="")||($profit_id==''||$profit_id=='0'))
           {
               $msg = app::get('b2c')->_('业务员必须选择业务类型和佣金规则！');
               $fag = 0;
           }
        }
         //如果选择了上家，上家只能是一级分销商，不能是二级
        if(!empty($data['member_pre_id'])&&$data['member_pre_id']!=0)
        {
            $pre_id=$data['member_pre_id'];
            $pre_obj=$this->get_member_type($pre_id);
            if(!empty($pre_obj)){
                 $msg = app::get('b2c')->_('您选择的上家是二级分销会员，不允许添加下级分销会员，请选择一级分销会员成为上家或为空！');
                 $fag = 0;
            }
        }
        return $fag;
    ...
    //获取所有会员业务类型为1的会员
      function get_member_type_id(){
        $row = $this->db->select('select m.*,t.name as type_name,a.login_account from sdb_b2c_members  as m '
                . 'left join sdb_b2c_member_type  as t  on m.member_type_id=t.member_type_id '
                . 'inner join sdb_pam_members as a on   a.member_id= m.member_id and a.login_type="local"'
                . 'and m.is_salesman=true');
         return $row;
      }
     //判断上级是否是一级
      function get_member_type($member_pre_id){
        $row = $this->db->select('select * from sdb_b2c_members where member_pre_id='
           . '(select  member_pre_id from sdb_b2c_members where member_id='.$member_pre_id.' and member_pre_id>0)');
         return $row;
      } 
    //
    /**
     * 通过会员的编号得到member标准数据格式
     * @params string member id
     * @params string page number
     * @params array order status
     * @return array sdf 数据
     */
    public function fetchByMember($member_id, $nPage=1, $filter_pre=array(),$limit=10)
    {
        ...
        if (!$filter_pre)
            $filter = array('up_member_id' => $member_id);
        else{
            $filter=$order_status;
        }
        $sdf_orders = $this->getList('*', $filter, $limitStart, $limit, 'up_member_id DESC');  
        // 生成分页组建
     ...
     * @return array sdf 数据
     */
    public function fetchByOrderList($member_id, $nPage=1, $filter_pre=array(),$limit=10)
    {
        #$limit = $this->app->getConf("selllog.display.listnum");
        if (!$limit)
            $limit = 10;
        $limitStart = ($nPage-1) * $limit;
        $orders=$this->get_sale_order($member_id); 
         if (isset($orders))
         {
           $filter = array('order_id' => $orders);
           $sdf_orders =app::get('b2c')->model('orders')->getList('order_id,pay_status,status,ship_status,received_status,createtime,member_id,final_amount',$filter, $limitStart, $limit, 'createtime DESC'); 
           if($sdf_orders)
           {
             foreach ($sdf_orders as $key => $value) {
                $value['pay_status'] =$value['pay_status']==null?"未付款":app::get('b2c')->model('orders')->trasform_status("pay_status",  $value['pay_status']);
                $value['status'] =app::get('b2c')->model('orders')->trasform_status("status", $value['status']);
                $value['shipping_status'] =$value['shipping_status']==null?"未发货":app::get('b2c')->model('orders')->trasform_status("shipping_status", $value['shipping_status']);
                $value["createtime"]=date('Y-m-d H:i:s', $value["createtime"]);
                $username=app::get('pam')->model('members')->getRow('login_account', array('member_id'=>$value["member_id"], 'login_type'=>'local'));
                $mprofit["member_profit_id"]=app::get('b2c')->model('members')->getRow('member_profit_id ',array('member_id'=>$member_id));//查询我的佣金规则
                $profit=app::get('b2c')->model('member_profit')->getRow('profit1', array('profit_id'=> $mprofit["member_profit_id"]));
                //根据订单id和本人用户id查看佣金总额以及是否分成
                $profit_sale=app::get('b2c')->model('member_log')->getRow('money,status', array('member_id'=> $member_id,'order_id'=>$value["order_id"]));
                $arr[] = array('order_id' => $value['order_id'],
                    ...
                    'username' => $username['login_account'],
                    'profit1' => $profit['profit1'],
                    'salemoney' => $profit_sale['money'],
                    'salestatus' => $profit_sale['status'] > 0 ? "<font style='font-weight:bold'>已分成</font>" : "<font style='color:red;font-weight:bold'>未分成</font>",
                    'url' =>"" ,
                );
            }
           }
        ...
        // 生成分页组建
        $countRd = app::get('b2c')->model('orders')->count($filter);
        $total = ceil($countRd/$limit);
        $current = $nPage;
        $token = '';
        $arrPager = array(
    ...
    //查询经销商的订单
     function get_sale_order($member_id){
        $row = $this->db->select('select  order_id from sdb_b2c_member_order_info where member_id='.$member_id);
      ...
      //查询经销商的销售总额
     function get_sale_amount($member_id){
         $sql='select sum(amount) as amount from  sdb_b2c_member_sale where  `status`=1 and  member_id='.$member_id;
