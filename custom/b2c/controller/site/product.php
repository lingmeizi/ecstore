...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    private function _get_goods_setting($goods_id){
        ...
        // 20161227 add by ldward 判断是否推荐

        $userObject = kernel::single('b2c_user_object');
        $siteMember = $userObject->get_current_member();
        if( empty($siteMember['member_id']) ){
            $setting['isshare'] ='false';//是否推荐
        }else{

            $setting['isshare'] ='true';//是否推荐
        }
        //20161227 end by ldward
    ...
    public function index() {
        ...
        //add by ldward 20161229
        if(count($_getParams)==2)
        {

            $memberId = (int)$_getParams[1];
            $_SESSION['member_id'] = $memberId.','.$goodsId;
        }
        //end by ldward
        ...
        $this->pagedata['async_request_list'] = $this->get_body_async_url($productBasic);
         ...
         //获取当前用户名，重新定义分享的url add by dlh(修改)
        $userObject = kernel::single('b2c_user_object');
        $wap_share=array('app'=>'b2c','ctl'=>'wap_product','act'=>'index','full'=>1,'arg0'=>$productId);
        $site_share=array('app'=>'b2c','ctl'=>'site_product','act'=>'index','arg0'=>$productId);
        if($userObject->is_login())
        {
            $member_id = $userObject->get_member_session();
            $wap_share=array('app'=>'b2c','ctl'=>'wap_product','act'=>'index','full'=>1,'arg0'=>$productId,'arg1'=>$member_id);
            $site_share=array('app'=>'b2c','ctl'=>'site_product','act'=>'index','arg0'=>$productId,'arg1'=>$member_id);        
        }
        //分享到微信的url
        if( app::get('wap')->is_actived() ){
            $share_wx_url = app::get('wap')->router()->gen_url($wap_share);
            //$share_wx_url =kernel::single('wap_router')->gen_url($wap_share);
        }
        $this->pagedata['share_wx_url'] = $share_wx_url;
        $share_url= $this->gen_url($site_share);
        $this->pagedata['share_url'] ='http://'.$_SERVER['HTTP_HOST'].$share_url;
    ...
    */
    ...
    public function test(){
    	...
    	$params = array(
            'method' => 'b2c.order.sooutputcustoms',
            'MerchantOrderID' => '170221145884830',
            'Status' => 1,
            'ShipTypeID' => '2',
            'TrackingNumber' => '23424234',
            'CommitTime' => $time,
            'Message' => '测试',
        );
        $url = "http://120.76.40.162:8101/index.php/api";
        $token = "fb6fa6d19beaf97c3912cfbf253c4aade3175dd081c34ca0f483f4a0d10798dc";
        $params['sign'] = strtoupper(md5(strtoupper(md5($this->assemble($params))).$token));
