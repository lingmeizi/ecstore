...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    private function get_cpmenu(){
        ...
        array(
           'label'=>app::get('b2c')->_('分销管理'),
           'mid'=>4,
           'items'=>array(
               array('label'=>app::get('b2c')->_('我的分享'),'app'=>'b2c','ctl'=>'site_member','link'=>'myshare'),
               array('label'=>app::get('b2c')->_('我的会员'),'app'=>'b2c','ctl'=>'site_member','link'=>'custom'),
               array('label'=>app::get('b2c')->_('佣金明细'),'app'=>'b2c','ctl'=>'site_member','link'=>'my_profit'),
               array('label'=>app::get('b2c')->_('提现明细'),'app'=>'b2c','ctl'=>'site_member','link'=>'my_back'),
               array('label'=>app::get('b2c')->_('佣金提现'),'app'=>'b2c','ctl'=>'site_member','link'=>'profit'),
               array('label'=>app::get('b2c')->_('分销商申请'),'app'=>'b2c','ctl'=>'site_member','link'=>'addsupplier'),
           ),
       ),
        );
       ...
       //我的分销只有业务员可以看
        ...
        if($member["isjxs"]=="否")
        {
            unset($arr_bases[4]["items"][0]);
            unset($arr_bases[4]["items"][1]);
            unset($arr_bases[4]["items"][2]);
            unset($arr_bases[4]["items"][3]);
            unset($arr_bases[4]["items"][4]);
        }
    ...
    function store(){
        ...
        $this->path[] = array('title'=>app::get('b2c')->_(''),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;

        $store = app::get('b2c')->model('store');
        $stores = $store->getList("*");

        $this->pagedata['stores'] = $stores;
        $this->pagedata['num'] = count($stores);
        $this->output();
    }
    ...
    function storedetail($store_id=0){
        ...
        $store = app::get('b2c')->model('store');
        $stores = $store->getList("*",array("store_id"=>$store_id));
  ...
  //}
    ...
    */
    ...
    //add by dlh
    //我的客户
    function custom($nPage=1){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的会员'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $member = $this->app->model('members');
        $aData = $member->fetchByMember($this->app->member_id,$nPage);
         //循环查询用户名邮箱密码
        $pamMembers=app::get('pam')->model('members');
        $members=$aData['data'];
        foreach($members as  $key=>$values)
        {
            $username = $pamMembers->getRow('login_account', array('member_id'=>$values["member_id"], 'login_type'=>'local'));
            if(isset($username))
            {
                $members[$key]["username"]=$username["login_account"];
            }
            $email = $pamMembers->getRow('login_account', array('member_id'=>$values["member_id"], 'login_type'=>'email'));
            if(isset($email))
            {
                $members[$key]["email"]=$email["login_account"];
            }
            $mobile = $pamMembers->getRow('login_account', array('member_id'=>$values["member_id"], 'login_type'=>'mobile'));
            if(isset($mobile))
            {
                $members[$key]["mobile"]=$mobile["login_account"];
            }
             $members[$key]["regtime"]=date('Y-m-d H:i:s', $values["regtime"]); 
        }
        $this->pagedata['my_members'] = $members;
        $this->pagination($nPage,$aData['pager']['total'],'custom');//当前页，总页数，方法名称，参数
        $this->pagedata['res_url'] = $this->app->res_url;
        $this->output();
    }
    //我的订单
    function my_order_list($nPage=1){
          $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的订单'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $member = $this->app->model('members');
        $aData = $member->fetchByOrderList($this->app->member_id,$nPage);
         //循环查询用户名邮箱密码
        $this->pagedata['order_lists'] = $aData['data'];
        $this->pagination($nPage,$aData['pager']['total'],'order_list');//当前页，总页数，方法名称，参数
        $this->pagedata['res_url'] = $this->app->res_url;
        $this->output();
    }
    /* add by ldward 20170111
     *  佣金明细
     * $change_type  参数用于判断是佣金明细还是提现管理
     */
    function myshare()
    {
        $memberModel = $this->app->model('members');
        $memberData = $memberModel->getList('isjxs',
                array('member_id' => $this->app->member_id,
                    'isjxs'=>'1'),0,1);
       // var_dump($this->redirect());
        if($memberData[0]['isjxs']=='1')
        {    
            $res_url = $this->gen_url(array('app'=>'site', 'ctl'=>'default', 'act'=>'index','arg0'=>$this->app->member_id,'full'=>1));
            $share_wx_url= app::get('wap')->router()->gen_url(array('app'=>'wap', 'ctl'=>'default', 'act'=>'index','arg0'=>$this->app->member_id,'full'=>1));
        }else{
            $res_url= $this->gen_url(array('app'=>'site', 'ctl'=>'default', 'act'=>'index','full'=>1));
            $share_wx_url=app::get('wap')->router()->gen_url(array('app'=>'wap', 'ctl'=>'default', 'act'=>'index','full'=>1));
        }
        $this->pagedata['share_url']=$res_url;
...
//    }
     ...
     * $change_type  参数用于判断是佣金明细还是提现管理
     */
    function my_profit($nPage=1){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('佣金明细'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $aData = app::get('b2c')->model('member_sale')->fetchBysale($this->app->member_id,$nPage);
        $this->pagedata['my_profit'] = $aData['data'];
        $this->pagination($nPage,$aData['pager']['total'],'my_profit',array($change_type));//当前页，总页数，方法名称，参数
        $this->pagedata['res_url'] = $this->app->res_url;
        $this->output();
    }
     ...
     * $change_type  参数用于判断是佣金明细还是提现管理
     */
    function my_back($nPage=1){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('提现明细'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $aData = app::get('b2c')->model('member_back')->fetchByback($this->app->member_id,$nPage);
        $this->pagedata['my_profit'] = $aData['data'];
        $this->pagination($nPage,$aData['pager']['total'],'my_back');//当前页，总页数，方法名称，参数
        $this->pagedata['res_url'] = $this->app->res_url;
        $this->output();
    }
    //提现申请
    function profit(){
         //获取销售总额以及佣金总额
         // 总销售额
        $sale_money = app::get('b2c')->model('members')->get_sale_amount($this->app->member_id);
        $this->pagedata['sale_money_order'] = $sale_money ? $sale_money : '0.00';
        //佣金金额
        $sale_money  = app::get('b2c')->model('member_drp')->getRow("user_money",array("member_id"=>$this->app->member_id));
        $this->pagedata['sale_money'] = $sale_money["user_money"] ? $sale_money["user_money"] : '0.00';
        //已经申请，未通过金额
        $noaudit_money  = app::get('b2c')->model('member_back')->getmoney($this->app->member_id,0,0);
        $this->pagedata['noaudit_money'] = $noaudit_money ? $noaudit_money : '0.00';
        //佣金金额减去申请金额
        $this->pagedata['sur_money'] =$sale_money["user_money"]-$noaudit_money;
        //注册是否需要验证码
        $this->pagedata['valideCode'] = app::get('b2c')->getConf('site.register_valide');
        $this->output();
    }
    //提现申请
    function account_apply(){
        $amount = isset($_POST['amount']) ? floatval($_POST['amount']) : 0;
        //用于盘点金额
        $mombers=app::get('b2c')->model('members');
        $firstday = date('Y-m-01 00:00:00', strtotime(date("Y-m-d")));
        $fiveday = date('Y-m-05 23:59:59', strtotime(date("Y-m-d")));
        $nowday=date("Y-m-d H:i:s"); 
        if($nowday<$firstday||$nowday>$fiveday){
            $msg = app::get('b2c')->_('只有在每月1-5日才能发起申请哦!');
            $this->splash('failed',null,$msg,true);exit;;
        }
        if(app::get('b2c')->getConf('site.register_valide')=='true'&&!base_vcode::verify('LOGINVCODE',$_POST['signupverifycode'])){
            $msg = app::get('b2c')->_('验证码填写错误!');
            $this->splash('failed',null,$msg,true);exit;
         }
        if ($amount <= 0)
        {
            $msg = app::get('b2c')->_('提现金额必须大于0!');
            $this->splash('failed',null,$msg,true);exit;
        }
        /* 判断是否有足够的余额的进行退款的操作 */
        //$sur_amount  = app::get('b2c')->model('members')->getRow("user_money",array("member_id"=>$this->app->member_id));
        $sur_amount = isset($_POST['sur_money']) ? floatval($_POST['sur_money']) : 0;
        if ($amount > $sur_amount)
        {
            $msg = app::get('b2c')->_("佣金金额不足")."<br />";
             $this->splash('failed',null,$msg,true);exit;
        }
        /* 插入帐户变动记录 */
        $account_log = array(
             'members'   =>array("member_id"=>$this->app->member_id) ,
             'member_name'   =>kernel::single('b2c_user_object')->get_member_name() ,
             'money'    =>   $amount,
             'balance_money'=>($sur_amount-$amount),
             'change_time'   => time(),
             'bank_info'      =>  $_POST['bank_info'],
         );
         app::get('b2c')->model('member_back')->save($account_log);
         $msg = app::get('b2c')->_("提现成功，等待审核...")."<br />";
         //刷新本页
         $url=array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'my_back');
         $this->splash('success',$url);
    }
    ...
    function addsupplier(){
        ...
        $GLOBALS['runtime']['path'] = $this->path;
        //根据会员信息查询分销商信息
        $momber_drp=app::get('b2c')->model('member_drp');
        $membersData=$momber_drp->getRow("*",array("member_id"=>$this->app->member_id));
        $membersData["appstatus"]=$momber_drp->getStatus($membersData['appstatus']);
        //相关会员信息
        $userObject = kernel::single('b2c_user_object');
        $datas=$userObject->get_current_member();
        //$membersData = $userObject->get_pam_data('*',$this->app->member_id);
        $membersData['license'] = app::get('b2c')->getConf('b2c.register.setting_memberlicense');
        $membersData['isjxs']=$datas['isjxs'];
        if(!$membersData["gender"])
        {
            $membersData["gender"]='female';
        }
        //查询图片
        $oImage_attach = app::get('image')->model('image_attach');
        $arr_image_attach = $oImage_attach->getRow('*',array('target_id'=>$this->app->member_id,'target_type'=>'drp'));
        $membersData["image_id"]=$arr_image_attach["image_id"];
        $this->pagedata['mem'] = $membersData;
        $this->pagedata['basc_url'] = 'http://'.$_SERVER['HTTP_HOST']."/custom/b2c/images/yes.gif";
        $this->pagedata['typeList'] = array('1'=>app::get('b2c')->_("有"), '0'=>app::get('b2c')->_("无"));
        $this->pagedata['isStoreList'] = array('1'=>app::get('b2c')->_("有店铺分销商"), '0'=>app::get('b2c')->_("无店铺分销商"));
        $this->output();
    }
        //图片处理
        function uploadimages(){
            ...
            if($_FILES["photoimg"]['size']>2048000){
                    echo '图片过大！';
            }
            $image_id  = $mdl_img->store($_FILES["photoimg"]['tmp_name'],null,null,$image_name);
            if(!$image_id) {
                    logger::info("errorrrr");
            }
            $mdl_img->rebuild($image_id,array('L','M','S'));
            $image_src_s = base_storager::image_path($image_id,'s');
            $image_src_old = base_storager::image_path($image_id);
            return array('image_id'=>$image_id,'src'=>$image_src_s,'src_old'=>$image_src_old);
        }
    //提交经销商
    function appsupplier()
    {
         //添加经销商信息
        $url=$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'addsupplier','full'=>1));
        $member_drp  =app::get('b2c')->model('member_drp')->getRow("*",array("member_id"=>$this->app->member_id,"appstatus"=>2));
        //var_dump($member_drp);
        if($member_drp)
        {
              $this->splash('failed',null,'分销商申请已审核，不能修改分销商信息！');
        }else{
            $_POST['phone']['telephone']=$_POST["tel"];//添加电话，因为引用
            $_POST['profile']['gender']=$_POST["gender"];//添加性别
            $_POST['appstatus']["type"]=1;
            $_POST['birthday']=$_POST["birthday"];
            $_POST['appstatus']=1;
            $_POST['members']=array("member_id"=>$this->app->member_id);//用户id;
            $_POST['member_name']=kernel::single('b2c_user_object')->get_member_name();//用户名称 
            //增加图片数据表
            if(isset($_FILES) && is_array($_FILES) && !empty($_FILES)&&$_FILES["photoimg"]['name']!=""){	
                $imagemsg = $this->uploadimages($_FILES);
                $oImage_attach = app::get('image')->model('image_attach');
                if($imagemsg['image_id'] == ''){
                        $msg = app::get('b2c')->_('图片上传失败！请重新上传!');
                        $this->splash('failed',$url,$msg);exit;
                }else{
                       $arr_remove_image = array();
                        $arr_image_attach = $oImage_attach->getList('*',array('target_id'=>$this->app->member_id,'target_type'=>'drp'));
                        $post_image_id = array();
		    ...
		    if($imagemsg['image_id']){
                        ...
                        if ($arr_remove_image){
                            foreach($arr_remove_image as $_arr_remove_image)
                                $test = $oImage->delete_image($_arr_remove_image,'drp');
                            }
                            $oImage_attach->delete(array("target_id"=>$this->app->member_id));
			}
                      $subf=array(
                          "target_id"=>$this->app->member_id,
                          "target_type"=>"drp",
                          "image_id"=>$imagemsg['image_id'],
                          "last_modified"=>time()
                      );
                      $oImage_attach->save($subf);//添加图片
                    }
		}
             $this->save();//保存经销商信息
        }
//        $this->output();   
    ...
    function save(){
        $this->begin();
        $objMemProfit = $this->app->model('member_drp');
        if($_POST['member_drp_id']){
            $olddata = app::get('b2c')->model('member_drp')->dump($_POST['member_drp_id']);
        }
       $url=$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'addsupplier','full'=>1));
        ...
        if($objMemProfit->save($_POST)){
            ...
            if($obj_operatorlogs = kernel::service('operatorlog.members')){
                if(method_exists($obj_operatorlogs,'member_drp_log')){
                    $newdata = app::get('b2c')->model('member_drp')->dump($_POST['member_drp_id']);
...
//            }else{
              //跳转页面
             // $this->splash('success',$url,'提交成功！',true);exit;    
              $this->end(true,app::get('b2c')->_('提交成功'),$url);  
//            }
        }else{
           // $this->splash('failed',$url,'提交失败！',true);exit;
            $this->end(false,app::get('b2c')->_('提交失败'),$url);
       }
    }
    
    //经销商须知
    public function license(){
        $this->pagedata['reg_license'] = app::get('b2c')->getConf('b2c.register.setting_memberlicense');
        $this->page('site/member/license.html');
    }
}
