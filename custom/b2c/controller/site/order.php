...
class b2c_ctl_site_order extends b2c_frontpage{
    ...
    public function create()
    {
        ...
        // add by ldward 20161228 添加判断
        
        //$this->end(false, '123', '',true,true);
        ...
        if ($result)
        {
          ...
          //add by ldward
          //计算佣金规则,如果本人是经销商算佣金，下级会员购买算佣金
            if($arrMember["isjxs"]=='是')//如果本人是经销商购买获得佣金
            {
              //使用订单金额计算佣金明细
              app::get("b2c")->model('member_sale')-> update_order_sale($order_id,$arrMember["member_id"]);
            }
           else if(!empty($arrMember["up_member_id"])&&$arrMember["up_member_id"]>0)//会员购买上级经销商获得佣金,会员按折扣计算
            {
              //使用订单金额计算佣金明细
              app::get("b2c")->model('member_sale')-> update_order_sale($order_id,$arrMember["up_member_id"]);
            }
          //end by dlh
