...
class b2c_ctl_wap_cart extends wap_frontpage{
    ...
    public function checkout_result($isfastbuy=0){
        ...
        if ($total_amount < 0) $total_amount = 0;
         ...
         //如果有存在折扣的话就减去折扣 add by dlh
         $member_up = app::get("b2c")->model('members')->getRow('up_member_id,isjxs',array('member_id'=>$arrMember['member_id']));
         if(!empty($member_up["up_member_id"])&&$member_up["up_member_id"]>0&&$member_up["isjxs"]=='0'&&$total_amount>0&&$total_amount>$this->pagedata['aCart']['discount_useramount'])//会员购买按折扣计算
         {
          $total_amount=$total_amount-$this->pagedata['aCart']['discount_useramount'];
          }
        //如果有存在折扣的话就减去折扣 end by dlh
        // 是否可以用积分抵扣
        ...
        $this->pagedata['order_detail'] = array(
            ...
            'pmt_discount' =>$this->pagedata['aCart']["discount_useramount"],
        ...
        // 如果是下级会员且不是经销商显示会员折扣
       if(!empty($member_up["up_member_id"])&&$member_up["up_member_id"]>0&&$member_up["isjxs"]=='0')
         {
          $total_amount=$total_amount-$this->pagedata['aCart']['discount_useramount'];
          $this->pagedata['up_member_id'] = "true";
          }
        //会员积分
    ...
    public function total(){
        ...
        // 如果是下级会员且不是经销商显示会员折扣
       if(!empty($arrMember["up_member_id"])&&$arrMember["up_member_id"]>0&&$arrMember["isjxs"]=='否')
        {
          $this->pagedata['up_member_id'] = "true";
        }
