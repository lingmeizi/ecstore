...
class base_rpc_service{
    ...
    private function parse_rpc_request($request){
        //如果传过来的参数包含method=Order.SOOutputCustoms，就重新组合参数
       //add by dlh
       if($request['method']=="Order.SOOutputCustoms")
       {
        //把data转换成数组
        $sdf= json_decode($request["data"]);
        $sdf_data["Status"]=$sdf->Status;
        $sdf_data["ShipTypeID"]=$sdf->ShipTypeID;
        $sdf_data["TrackingNumber"]=$sdf->TrackingNumber;
        $sdf_data["CommitTime"]=$sdf->CommitTime;
        $sdf_data["Message"]=$sdf->Message;
        $sdf_data["MerchantOrderID"]=$sdf->MerchantOrderID;        
        $sdf_data["method"]=$request["method"];
        $sdf_data["sign"]=$request["sign"];
        unset($request["sign"]);
        //计算签名为了对比
        $secretkey=app::get('b2c')->getConf('b2c.register.setting_membersecretkey')?app::get('b2c')->getConf('b2c.register.setting_membersecretkey'):"kjt@3053";//appSecret
        $sign_check_so =md5($this->assemble($request,$secretkey));
        //直接加入我们的验证规则的验证码
        $request=$sdf_data;
      }
        //end by dlh
    ...
    //添加第三方验证签名规则
    function assemble($params,$secretkey){
    ...
    private function process_rpc(){
        ...
        if($method=="SOOutputCustoms")
        {
            if($status!='succ'&&$result=="")
            {
                echo "FAILURE";
            }else{
            echo $result;
