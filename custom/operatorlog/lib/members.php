...
class operatorlog_members{
     ...
     * @param type $olddata
     */
    function member_type_log($newdata,$olddata){
        if(empty($_POST['member_type_id'])){
            $this->objlog->logs('goods', '添加会员业务类型', '添加会员业务类型 '.$_POST['name']);
        }else{
            $modify_flag = 0;
            $data = array();
            foreach($newdata as $key=>$val){
            ...
            if($modify_flag>0){
                $memo  = "serialize".$this->delimiter."编辑会员业务类型ID {$_POST['member_type_id']}".$this->delimiter.serialize($data);
                $this->objlog->logs('goods', '编辑会员业务类型', $memo);
            }
        }
    }
     ...
     * @param type $olddata
     */
    function member_profit_log($newdata,$olddata){
        if(empty($_POST['member_profit_id'])){
            $this->objlog->logs('goods', '添加会员佣金规则', '添加会员佣金规则 '.$_POST['name']);
        }else{
            $modify_flag = 0;
            $data = array();
            foreach($newdata as $key=>$val){
            ...
            if($modify_flag>0){
                $memo  = "serialize".$this->delimiter."编辑会员佣金规则ID {$_POST['member_profit_id']}".$this->delimiter.serialize($data);
                $this->objlog->logs('goods', '编辑会员佣金规则', $memo);
            }
        }
    }
